...
\subsection{V1 — La restitution spatiale : ce que l'espace exige et ce qu'il pardonne}
...
\subsection{V2 — La mise en relation documentaire : le vocabulaire comme obstacle}
...
de correspondances faibles — du bruit —, tandis que la recherche reformulée fait
...
\subsection{V3 — L'ancrage documentaire : ce que l'assistant refuse de dire}
...
\subsection{V4 — Le cloisonnement : pourquoi la garantie doit être portée par la base}
...
\footnotesize
\begin{tabularx}{\textwidth}{|C{1.2cm}|X|C{2.6cm}|}
\hline
\rowcolor{keycetete}
\thead{Hyp.} & \thead{Éléments de vérification} & \thead{Conclusion} \\ \hline
\textbf{HS1} &
...
\begin{figure}[htbp]
\centering
\aplacer{7.2cm}{Représenter le modèle des quatre continuités. Placer au centre
la \emph{Qualité de l'expérience de visite à distance} et, autour, les quatre
continuités : \textbf{spatiale} (panorama 360, points de passage),
\textbf{d'échelle} (3D, réalité augmentée à taille fixe),
\textbf{documentaire} (ancrage sur les documents, citation des sources, aveu
d'ignorance) et \textbf{narrative} (objet $\rightarrow$ chef $\rightarrow$
lignée $\rightarrow$ œuvres apparentées). Faire figurer, sous chaque continuité,
le dispositif technique qui la porte et la rupture qui la menace. Indiquer que
la rupture d'une seule continuité dégrade l'ensemble.}
\caption{Modèle des quatre continuités de la visite restituée}
\label{fig:modele}
\source{Élaboré par nos soins à partir des résultats de l'étude.}
\end{figure}
...
\begin{enumerate}
  \item[\textbf{OI1.}] Industrialiser la chaîne de production par la
        conteneurisation et l'orchestration, avec des environnements de
        pré-production et de production distincts.
  \item[\textbf{OI2.}] Consolider la chaîne documentaire de l'assistant, de
        l'ingestion des documents à l'indexation vectorielle.
  \item[\textbf{OI3.}] Produire les médias manquants : prises de vue à
        360~degrés de l'ensemble des espaces et numérisation d'un premier
        ensemble d'œuvres.
  \item[\textbf{OI4.}] Transférer au personnel la maîtrise de l'outil au moyen
        d'une formation et d'une documentation d'exploitation.
  \item[\textbf{OI5.}] Accueillir deux institutions partenaires afin d'éprouver
...
\begin{figure}[htbp]
\centering
\aplacer{9.5cm}{Représenter la chaîne DevOps cible. De gauche à droite :
\textbf{Développeur} $\rightarrow$ \texttt{git push} $\rightarrow$ \textbf{Dépôt
GitHub} $\rightarrow$ \textbf{GitHub Actions — Intégration continue}
(installation, compilation, contrôles, analyse de sécurité des dépendances,
\texttt{terraform validate}) $\rightarrow$ \textbf{Construction de l'image
Docker} (étape 1 : compilation Node ; étape 2 : diffusion Nginx) $\rightarrow$
\textbf{Amazon ECR} (registre d'images, analyse de vulnérabilités)
$\rightarrow$ \textbf{Déploiement} par jeton OIDC de courte durée vers
\textbf{Amazon ECS Fargate} — environnement \emph{pré-production} puis, après
validation, environnement \emph{production} — avec \textbf{Application Load
Balancer} en frontal, \textbf{CloudFront} en diffusion et \textbf{Route 53} pour
les noms de domaine. Ajouter les briques de support : \textbf{CloudWatch}
(journaux, métriques, alarmes), \textbf{AWS Cost Explorer / FinOps} (suivi de la
dépense), \textbf{Terraform} (description de l'ensemble des ressources), et la
voie d'évolution en pointillés vers \textbf{Amazon EKS} (Kubernetes) pour la
montée en charge. Faire figurer les tâches planifiées : ingestion documentaire
vers l'index vectoriel et reconstruction photogrammétrique. Indiquer le retour
automatique à la version précédente en cas d'échec de la vérification.}
...
sont présentés au tableau~\ref{tab:orchestrateur}.
...
\begin{enumerate}
  ...
  \item \textbf{Éprouver la chaîne complète de rapprochement en conditions
        ...
        complet — mise en cache, vectorisation, reclassement, proposition —
        n'a jamais été exécuté de bout en bout avec un compte du personnel. Cet
...
\label{tab:planning}
\small
\begin{tabularx}{\textwidth}{|C{1.2cm}|X|C{2.4cm}|C{2.6cm}|}
\hline
\rowcolor{keycetete}
\thead{Obj.} & \thead{Action} & \thead{Durée} & \thead{Responsable} \\ \hline
OI1 & Conteneurisation et registre d'images & 2 semaines & Équipe technique \\ \hline
OI1 & Environnements de pré-production et de production & 3 semaines & Équipe technique \\ \hline
OI1 & Journalisation, alarmes et suivi de la dépense & 1 semaine & Équipe technique \\ \hline
OI2 & Automatisation de l'ingestion documentaire & 2 semaines & Équipe technique \\ \hline
OI2 & Essai de bout en bout du rapprochement & 1 semaine & Équipe technique \\ \hline
OI2 & Demandes de clés d'accès & 4 semaines & Direction \\ \hline
OI3 & Campagne de prises de vue à 360 degrés & 3 semaines & Pôle conservation \\ \hline
OI3 & Numérisation des œuvres emblématiques & 6 semaines & Pôle conservation \\ \hline
OI4 & Formation du personnel et documentation & 2 semaines & Équipe technique \\ \hline
OI5 & Accueil de deux institutions partenaires & 4 semaines & Direction \\ \hline
\end{tabularx}
\source{Élaboré par nos soins.}
\end{table}
...
\begin{figure}[htbp]
\centering
\aplacer{6.5cm}{Insérer un diagramme de Gantt reprenant les dix actions du
tableau~\ref{tab:planning} sur un axe de temps de six mois, en distinguant par
la couleur les cinq composantes de l'intervention et en faisant apparaître les
dépendances entre actions.}
\caption{Diagramme de Gantt de l'intervention proposée}
\label{fig:gantt}
\source{Élaboré par nos soins.}
\end{figure}
...
Elle est acquise. L'ensemble des briques nécessaires existe et a été éprouvé :
...
\subsubsection{Faisabilité économique}

Le tableau~\ref{tab:coutfonctionnement} présente le coût de fonctionnement
annuel projeté, dans l'hypothèse de la chaîne cible.
...
\encadre{Recommandation FinOps}{%
Trois mesures conditionnent le maintien de la dépense dans l'enveloppe projetée :
\textbf{étiqueter} toutes les ressources par environnement et par institution
afin de savoir qui consomme quoi ; \textbf{poser des alarmes budgétaires} à
80~\% et à 100~\% de l'enveloppe mensuelle ; et \textbf{arrêter automatiquement}
les environnements de pré-production hors heures ouvrées. Ces trois mesures ne
coûtent rien et évitent la dérive silencieuse qui caractérise les dépenses de
nuage.}

\subsubsection{Faisabilité organisationnelle}

Elle constitue le point le plus délicat. Elle suppose la désignation d'un
référent interne, la formation effective du personnel, et l'inscription de la
production des médias dans les activités courantes de la Fondation. La
composante~4 y pourvoit, mais son succès dépend d'une décision institutionnelle
