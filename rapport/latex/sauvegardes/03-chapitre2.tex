...
\begin{figure}[htbp]
\centering
\aplacer{5.2cm}{Représenter le tableau Scrumban employé pendant le stage.
Dessiner cinq colonnes : \textbf{Backlog} (réservoir des tâches issues du cahier
des charges), \textbf{À faire cette semaine} (sélection arbitrée avec
l'encadreur), \textbf{En cours} avec la mention « limite : 2 tâches »,
\textbf{En vérification} (contrôles automatiques et essais) et \textbf{Terminé}.
Faire figurer quelques cartes d'exemple issues du projet (moteur de panorama,
assistant conversationnel, cloisonnement des données). Ajouter, au-dessus des
colonnes, la flèche du flux continu venant du Kanban, et en dessous la boucle de
\emph{revue hebdomadaire avec l'encadreur} venant du Scrum, qui repart du
Backlog. Indiquer que le passage de « En vérification » à « Terminé » est
conditionné par le succès de la chaîne d'intégration continue.}
