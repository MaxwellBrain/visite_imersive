...
- Terraform ≥ 1.6
- Une zone Route 53 existante et déléguée (`nexacoode.space`)
...
```
nexacoode.space                     <- votre zone, deja deleguee, intacte
  musea.nexacoode.space             <- le site
    bandjoun.musea.nexacoode.space  <- une organisation (joker)
```
...
>
> ```bash
> nslookup -type=NS nexacoode.space 8.8.8.8
> ```
>
...
```bash
terraform output site_domain      # musea.nexacoode.space
...
- **Site URL** : `https://musea.nexacoode.space`
- **Redirect URLs** : ajouter `https://musea.nexacoode.space/**` **et** `https://*.musea.nexacoode.space/**`

Le second motif est indispensable : sans lui, un visiteur qui se connecte depuis
`bandjoun.musea.nexacoode.space` est renvoyé sur le mauvais domaine.
...
**Variables** : `PLATFORM_DOMAIN` = `musea.nexacoode.space`, `AWS_REGION` = `eu-west-3`
...
curl -o /dev/null -w "%{http_code}\n" https://musea.nexacoode.space/site/ar/demo   # attendu 200

# Le type MIME du modèle 3D — sinon la RA échoue en silence sur mobile
curl -sI https://musea.nexacoode.space/modeles/tabouret.glb | grep -i content-type  # model/gltf-binary

# Un sous-domaine d'organisation
curl -o /dev/null -w "%{http_code}\n" https://bandjoun.musea.nexacoode.space/       # attendu 200
```
...
**Un joker ne couvre qu'un seul niveau.** `*.musea.nexacoode.space` ne couvre pas
`musea.nexacoode.space` : le certificat déclare les deux, et ce n'est pas une redondance.
